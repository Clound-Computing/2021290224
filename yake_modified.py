...
class YAKE(LoadFile):
    ...
    def candidate_selection(self, n=3, stoplist=None, **kwargs):
        ...
        self.candidate_filtering(stoplist=list(string.punctuation), minimum_length=1, minimum_word_size=1)

        def havenumber(s):
            ZHnumber = "零一二三四五六七八九十百千万亿"
            f = False
            for i in s:
                if ((i >= '0') and (i <= '9')) or (i in ZHnumber):
                    f = True
                    break
            return f
        ...
        for k in list(self.candidates):
            ...
            # get the candidate
            v = self.candidates[k].surface_forms[0][0].lower()

            # filter candidates starting/ending with a stopword 
            if v in stoplist or '某' in v or havenumber(v) or 'x' in v or '×' in v:
    ...
    def _feature_extraction(self, stoplist=None):
        ...
        for word in self.words:
            ...
            self.features[word]['isstop'] = word in stoplist
            # Term Frequency
    ...
    def candidate_dict(self):
        candidate_sf = Counter()
        for k, v in self.candidates.items():
            lowercase_forms = [' '.join(t).lower() for t in v.surface_forms]
            for i, candidate in enumerate(lowercase_forms):
                candidate_sf[candidate] = 1
        return candidate_sf

    def candidate_weighting(self, candidate_df, N, window=2, stoplist=None, use_stems=False):
        ...
        for k, v in self.candidates.items():
            ...
            # use words
                ...
                for i, candidate in enumerate(lowercase_forms):
                    ...
                    if candidate in candidate_df:
                        df = candidate_df[candidate]
                    else:
                        df = 0
                    self.weights[candidate] /= math.log((N + 1) / (1 + df), 2)
    ...
    def get_n_best(self,
        ...
        for u in best[:min(n, len(best))]:
            if self.weights[u] < 1 or len(n_best) == 0:
                n_best.append((u, self.weights[u]))
            else:
                n_best.append(n_best[0])
